...
gboolean tweets_response_is_empty(const gchar *json_data);
gboolean profile_replies_response_is_empty(const gchar *json_data);
...
struct Conversation* parse_conversation_details(const gchar *json_data);
struct Community* parse_community_details(const gchar *json_data);
...
gchar* construct_tweet_payload(const gchar *content, const gchar *reply_to_id, GList *attachments);
